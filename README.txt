...
I. Introduction
	...
	Sunscreen calculates the spectrum of a galaxy whose stars are screened (rendered invisible) according to a cut based on physical properties of the stars.  The available cuts are stellar luminosity, luminosity-to-mass ratio, mass, and evolutionary stage.  It then calculate synthetic photometry from these spectra, both in the rest frame and redshifted according to the galaxy's age.  If you want to know where a single age stellar population would appear on a color-color diagram if all of its stars brighter than 100 L_sun were blotted out, or what the spectrum is of the AGB stars in a galaxy with constant star-formation, Sunscreen is your program.
...
II. Setup
	...
	i.) Download Sunscreen-Base-v1_2_1.zip and unzip it in the parent directory where you want Sunscreen to be placed.
	ii.) Download Isochrones.zip and unzip it in Sunscreen/CMD-Isochrones.
	iii.) Download StellarSpectra.zip and unzip it in Sunscreen/LejeuneSpectra.
	iv.) Download PhotometryZeroPoints.zip and unzip it in Sunscreen/FilterResponses.
	v.) Compile Sunscreen.c.  The code for Sunscreen itself is written in C and requires no special libraries.  On a UNIX-like environment, it can be compiled simply:
	...
	In addition, input data files -- namely, the star formation histories, stellar model spectra, stellar isochrones, and filter bandpasses -- need to be included in the directories:
		SFRs
		LejeuneSpectra
		CMD-Isochrones
		FilterResponses	
...
III. Input
	...
	The three values of metallicity for which Sunscreen	produces results are 0.1 Z_sun, 1.0 Z_sun, and 2.0 Z_sun.
	
	Changing them requires
		a.) Adding the subdirectories to the output directory structure (see the included CreateOutputDirectories.sh script).
		b.) Adding model stellar spectra for the desired metallicities to the LejeuneSpectra directory (see III-A).
		c.) Adding isochrones for the desired metallicities to the CMD-Isochrones directory (see III-B).
		d.) Changing the NZ constant to the desired number of metallicities to calculate results for, by looking for the "#define NZ" line in Sunscreen.c.
		e.) Changing the initial values of the logZ array in Sunscreen.c to the desired values.  
	...
	where metallicity (logZ) is given relative to Solar on a base-10 log scale (so Solar metallicity is +0.00, 1/10 Solar is -1.00), also denoted as [M/H]; temperature (Teff) is given without leading zeros in Kelvin; and surface gravity (logg) is given in cgs units, cm/s^2.  The format of the file is a text file with two columns: wavelength in cm, and emergent specific flux from the stellar surface (F_nu) in erg/cm^2/s/Hz.  The included ExtractLejeuneSpectra.sh script in the LejeuneSpectra directory can be used to create these files from downloadable BaSeL data.
	...
	Sunscreen uses the PARSEC v1.2S + COLIBRI PR16 isochrones available from the CMD 3.0 webserver.  These are stored in the CMD-Isochrones directory.  They are given in steps of log_10 t_Iso, where t_Iso is the age of the stars on the isochrone.  Currently the isochrones span log_10 (t_Iso / yr) from 4.00 to 10.13.  The lower limit is somewhat arbitrarily chosen, but generally predates any star falling on the main sequence.  The upper limit is given by the maximum age available from the CMD webserver.  log_10 t_Iso values from 4.00 to 10.12 were downloaded in two steps (4.00 to 7.00, 7.00 to 10.12) using the isochrone grid option, while 10.13 was not available this way and was downloaded as a single isochrone of age 13.4896 Gyr.  The number of isochrone ages is given in the NtIso constant.
	...
	where metallicity (logZ) is given in the [M/H] form, a base-10 logarithm relative to Solar, and age (logt) is given in the base 10 logarithm of age in years. Within each file is a table with several columns.  The columns are, in order: 
		(i) age in years
		(ii) initial stellar mass in M_sun 
		(iii) current stellar mass in M_sun
		(iv) base-10 logarithm of luminosity in L_sun
		(v) base-10 logarithm of effective temperature of the star in Kelvin
		(vi) base-10 logarithm of stellar radius in cm
		(vii) base-10 logarithm of stellar surface gravity in cm/s^2.
		(viii) an integer specifying the approximate stage of stellar evolution the star is in
		and (ix) [M/H], given as a check that the processing has occured correctly.  
	...
	Some things to be aware of:
		a.) The ages in these files are taken directly from the output* files from the CMD webserver.  As such, they only have three significant digits -- i.e., they are not calculated from the logt value in the file name.
		b.) Solar metallicity is assumed to be 0.0152, the default value for the current iteration of CMD.
		c.) Stellar radius is calculated from the star's surface gravity and current mass, under the star is spherical.
		d.) Sunscreen actually disregards the value of log L given in these files to ensure physical consistency.  Instead, Sunscreen calculate L using the Steffan-Boltzmann law: L = 4 pi R^2 * sigma_SB T_eff^4
		e.) The CMD isochrones for large t have a terminating point that correspond to a post-AGB star.  The luminosity of these "stars" is set to log L = -9.999, but the other physical parameters indicate that they are bright and blue.  According to the CMD 3.0 FAQ, they are only included to signal the end of the isochrone to some codes.  I regard them as spurious, and they are not included when ExtractIsochrones is run.
		f.) The CMD FAQ indicates that "stage" is only a rough indication. I've noticed that stars of mass around ~15 M_sun seem to remain in the pre-MS phase for much longer than stars of much higher or lower stellar masses.
	...
	To extend (or contract) the range of isochrone ages:
		i.) Set logtIsoMin to the new minimum of log_10 (t_Iso / yr).
		ii.) Set logtIsoMax to the new maximum of log_10 (t_Iso / yr).
		iii.) Set dlogtIso to the new increment in log_10 t_Iso.  Be aware that the AGB boosting phase described in Girardi et al. (2013) is resolved only if this is less than ~0.05.
		iv.) Change the NtIso constant to the new number of isochrones, which includes logtIsoMin and logtIsoMax, as given in the "#define NtIso" line.
	...
	B13-log10Mh+13.0
	B13-log10Mh+14.0
	These are average star formation histories from Behroozi et al. (2013) for present-day halo masses of 10^11, 10^12, 10^13, and 10^14 M_sun.  These correspond to the values plotted in the right panel of Figure 6, or those in the "sfh" directory of release-sfh_z0_z8_052913 downloaded from Behroozi's website.  The "sfr" directory contain SFRs of galaxies where the halo mass is measured at the observed redshift, not the present day.  The difference is that a halo that has M_h = 10^12 M_sun at z = 5 is going to be a lot bigger at z = 0.  Those values (plotted in the left panel of their Figure 6) do not correspond to the SFRs over time of individual galaxies.
	...
	The SFRs directory also includes a SFH, B13-log10Mh+15.0, for galaxies in present-day halo masses of 10^15 M_sun, but it is not calculated by Sunscreen.
	
	M15-MJAM+11.0
	This is one of the average star formation histories from McDermid et al. (2015), Figure 16 top panel, for early type galaxies as derived from the ATLAS3D survey.  It is the SFH for log_10 (M_JAM / M_sun) between 11.0 and 11.5 (yellow-orange line), where M_JAM is taken in that paper to be the stellar mass.  From what I can tell, the line in the plot is actually the rate that z = 0 stellar mass accumulates, not the total physical SFR.  I convert it to SFR using the prescription of Leitner & Kravtsov (2012) for a Chabrier IMF, which gives a present day stellar mass of 10^11 M_sun.  
	...
	For these auxiliary SFHs, Sunscreen integrates the mass formed between tBeginInt and tEndInt by calling TableSFH, where tBeginInt and tEndInt are still measured with respect to the present being t = 0.  Three additional variables must be supplied: NtSFH, the number of data points in the table, a tTable array storing a list of the times given in the table, and a SFRTable array storing SFRs in M_sun/yr at those times.  These will be handled automatically by the way Sunscreen reads in the files.
	
	To add another SFH in table form:
		i.) Place a SFH of the correct format somewhere like the SFRs directory where Sunscreen can read it.
		ii.) Change the NSFH variable to the correct number of SFHs, which is set in the "#define NSFH" line.
		iii.) Add the name of the SFH to be used in files to the SFModeName array.
		iv.) Add the path to the SFH file to the SFHTableFileNames array.
	...
	The format of the PhotonZeroPoint* files is a text file with two columns.  The first column is wavelength in cm.  The second column parameterizes the filter response and the zero point of the magnitude system for that filter.  To be more precise, the second column is the RECIPROCAL of the number flux of MONOCHROMATIC photons at that wavelength ONLY (in photons/cm^2/s) from a source with zero magnitude in that band.  Because it is a reciprocal, if a filter does NOT respond to the photons of that wavelength, this value will be zero; if it is highly responsive, the value will be relatively large.  
	...
	Note that Sunscreen does NOT include any additional correction for atmospheric extinction -- these must be accounted for in the calculations of the zero magnitude fluxes when producing these files. 
	
	To add a new filter for Sunscreen to calculate photometry in:
		i.) Place a PhotonZeroPoint-*.txt file corresponding to the filter response in the FilterResponses directory.
		ii.) Add the name of the filter, the same as used in the name of the PhotonZeroPoint file, to the FilterName array.
		iii.) Update the NFilt constant to the correct number of filters.  It can be found in the "#define NFilt" line.
	...
	The IMF is calculated by calling the IMF wrapper function.  It returns a quantity of the form dN/dM*, the number of stars per infitesimal stellar mass range per unit solar mass of stellar population.  All these masses are in units of M_sun.  Most of the time, Sunscreen doesn't call it directly, but calls IMFMassIntegral or IMFNumberIntegral to return the mass or number of stars between MLo and MHi per unit solar mass of stellar population.
	
	Three IMF functions exists, but there are six IMFs that Sunscreen is set up to calculate.  The three IMF functional forms are a Chabrier IMF, a Salpeter IMF, and a bottom-heavy IMF of the form dN/dM* ~ M*^-3.  At present, these are the three IMFs that Sunscreen calculates.  The other three IMFs are Chabrier IMFs, but with all star formation below 1 M_sun, 2 M_sun, and 5 M_sun suppressed.  

	For the Chabrier IMF, the normalization is set such that it gives the SFH currently being calculated.  This normalization is calculated first.  The Salpeter and bottom-heavy IMFs are then normalized so that the value dN/dM* (M* = 1 M_sun) is equal to that quantity's value for the Chabrier IMF using the precalculated normalization.  This normalization is chosen so that an old (~10 Gyr) stellar population will be roughly as bright for the Salpeter and bottom-heavy IMFs as for the Chabrier IMF, since most of the luminosity will be dominated by red giants of mass ~1 M_sun.  This means the true SFR assumed for these IMFs is NOT the value given by the SFHs.  The Chabrier IMFs with mass cutoffs have the same normalization as the Chabrier IMF.
	
	To add a new IMF:
		i.) Write a function that calculates the IMF (dN/dM* in units of M_sun, where N is the number of stars per unit solar mass of stellar population) and put it in Sunscreen.
		ii.) Put a new variable that will index this IMF in the enum IMFList, in the order that you want the IMF to be calculated.  For example, change this:
		...
		iii.) In the IMF (double, double, int) function, write a line of the form:
			if (IMFVariation == new_imf) return NewIMF (M, IMFNorm);
		The "new_imf" should be the same name you put in enum IMFList.
		iv.) Under the "Universal lookup tables" heading in Sunscreen's code, add a line that sets the new IMF's normalization and assigns it to its place in the IMFNorm array.  For example, the code for the bottom-heavy IMF is:
			IMFNorm[botheavy] = IMFNorm[botheavy] * IMF (1.0, 1.0, chabrier) / IMF (1.0, 1.0, botheavy);
		v.) Add the name of the IMF to the IMFName array, using the same order as IMFList.
		vi.) Update the NIMF constant to the number of IMFs that you want calculated, in the "#define NIMF" line.
	
	Sunscreen assumes the stellar population lies strictly between 0.1 and 120 M_sun.  These values are stored in MStarMin and MStarMax, respectively.  They are currently the same for all IMFs.  
	...
	To change the galaxy ages for which spectra are output:
		i.) Change the initial values of the logtGalaxyAge array to include the base 10 logarithms of the desired ages in years.  
		ii.) Update NtPop to the number of ages in this array, in the "#define NtPop" line.
		
	Remember that Sunscreen integrates stellar populations by stepping through each of the isochrones (section II-B).  Ages younger than 10^logtIsoMin or older than 10^logtIsoMax will not be calculated properly.  Sunscreen does NOT check this.
...
IV. Output
	...
	Models are stored in directories, sorted first according to the type of model, then metallicity, then IMF.  Results stored in each of the highest level directories are:
		...
		2. Small_logLtoM0_Screened_Results
			Results for stellar populations where all stars with (log) luminosity-to-initial-mass ratios below a threshold log (L/M_0) are screened.
		3. Small_logM0_Screened_Results
			Results for stellar populations where all stars with (log) initial mass below a threshold log M_0 are screened.
		...
		5. Large_logLtoM0_Screened_Results
			Results for stellar populations where all stars with (log) luminosity-to-initial-mass ratios above a threshold log (L/M_0) are screened.
		6. Large_logM0_Screened_Results
			Results for stellar populations where all stars with (log) initial mass above a threshold log M_0 are screened.
	...
	1. File names
	Sunscreen calculates the specific luminosity (L_nu) of the model galaxy (and its putative Dyson spheres) and writes them to files in the subdirectory specified by type of result, IMF, and then metallicity (see IV-A).  The name of the spectrum files has the format:
	...
	This is the spectrum of a galaxy where
		*Stars with a log (L/M_0) > 10^1.5 (L_sun/M_sun) are screened
		*The galaxy has a Salpeter IMF.
		*The galaxy's SFH is a single burst.
		*The galaxy has an age of 1 Gyr.
		*The galaxy has a metallicity of [M/H] = -1.0, or Z = 0.1 Z_sun.
	...
	The next column is the specific luminosity (L_nu) of the galaxy, after taking into account any screening of its stars.  It has units of erg/s/Hz.  
	...
	It is possible to change the number of megastructure temperatures for which this thermal emission is calculated and the values of those temperatures.  This can be done by:
		i.) Changing the ND constant to the number of desired output temperatures using the "#define ND" line.
		ii.) Changing the temperatures listed in the TDysonList array's initial values.
	...
	In converting from age to redshift, Sunscreen assumes a LambdaCDM cosmology with Omega_m = 0.315, H_0 = 67.31 km/s, and the radiation energy density of the Universe to be entirely contained in the CMB with temperature 2.73 K.  These values can be altered by changing the Omega_m_0, H_0, and TCMB_0 global variables, as well as the t0_Cosmic global variable giving the age of the Universe in seconds.  

	Unlike the spectra, each value of the threshold quantity, like log L, is given only one line in the photometry file.  So the photometry for models of mass thresholds of 0.01 M_sun, 1 M_sun, and 1000 M_sun are all given in the same file, if all other parameters (metallicity, IMF, SFH, age) are the same.  Essentially, the photometry files contain "tracks" of the galaxy in magnitude space as the threshold quantity is varied.
