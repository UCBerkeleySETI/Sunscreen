...
The three values of metallicity for which Sunscreenproduces results are 0.1 Z_sun, 1.0 Z_sun, and 2.0 Z_sun.
...
1. age in years
2. initial stellar mass in M_sun 
3. current stellar mass in M_sun
4. base-10 logarithm of luminosity in L_sun
5. base-10 logarithm of effective temperature of the star in Kelvin
6. base-10 logarithm of stellar radius in cm
7. base-10 logarithm of stellar surface gravity in cm/s^2.
...
* The ages in these files are taken directly from the output* files from the CMD webserver.  As such, they only have three significant digits -- i.e., they are not calculated from the logt value in the file name.
...
* Sunscreen actually disregards the value of log L given in these files to ensure physical consistency.  Instead, Sunscreen calculate L using the Steffan-Boltzmann law: L = 4 pi R^2 * sigma_SB T_eff^4
...
* The CMD FAQ indicates that "stage" is only a rough indication. I've noticed that stars of mass around ~15 M_sun seem to remain in the pre-MS phase for much longer than stars of much higher or lower stellar masses.
...
1. Set logtIsoMin to the new minimum of log_10 (t_Iso / yr).
2. Set logtIsoMax to the new maximum of log_10 (t_Iso / yr).
3. Set dlogtIso to the new increment in log_10 t_Iso.  Be aware that the AGB boosting phase described in Girardi et al. (2013) is resolved only if this is less than ~0.05.
...
Three IMF functions exists, but there are six IMFs that Sunscreen is set up to calculate.  The three IMF functional forms are a Chabrier IMF, a Salpeter IMF, and a bottom-heavy IMF of the form dN/dM&ast; ~ M&ast;^-3.  At present, these are the three IMFs that Sunscreen calculates.  The other three IMFs are Chabrier IMFs, but with all star formation below 1 M_sun, 2 M_sun, and 5 M_sun suppressed.  

For the Chabrier IMF, the normalization is set such that it gives the SFH currently being calculated.  This normalization is calculated first.  The Salpeter and bottom-heavy IMFs are then normalized so that the value dN/dM&ast; (M&ast; = 1 M_sun) is equal to that quantity's value for the Chabrier IMF using the precalculated normalization.  This normalization is chosen so that an old (~10 Gyr) stellar population will be roughly as bright for the Salpeter and bottom-heavy IMFs as for the Chabrier IMF, since most of the luminosity will be dominated by red giants of mass ~1 M_sun.  This means the true SFR assumed for these IMFs is NOT the value given by the SFHs.  The Chabrier IMFs with mass cutoffs have the same normalization as the Chabrier IMF.

To add a new IMF:
1. Write a function that calculates the IMF (dN/dM&ast; in units of M_sun, where N is the number of stars per unit solar mass of stellar population) and put it in Sunscreen.
